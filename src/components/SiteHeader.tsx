...
export function SiteHeader({ overlay = false }: Props) {
  ...
  useEffect(() => {
    ...
    const update = () => {
      const limit = anchor ? anchor.offsetHeight - 64 : 320;
      setScrolledPast(window.scrollY > Math.max(limit, 64));
    };
    update();
  ...
  return (
    <header
      className={`z-50 transition-colors duration-300 md:sticky md:inset-x-auto md:top-0 md:border-b md:border-[var(--nl-navy)]/10 md:bg-[var(--nl-white)]/95 md:backdrop-blur-sm ${
      ...
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-4 py-2">
        <Link href="/" className="relative shrink-0" aria-label="NeuroLinks home">
          <Image
            ...
            className={`w-auto transition-opacity duration-300 md:h-9 md:opacity-100 ${
              overlay ? "h-10" : "h-8"
          ...
          {overlay ? (
            <Image
              ...
              priority
              sizes={IMG_SIZES.logo}
              className={`absolute top-0 left-0 h-10 w-auto transition-opacity duration-300 md:hidden ${
